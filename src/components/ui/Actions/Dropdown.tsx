...
export const Dropdown = React.memo(({ options, value, onSelect, ...rest }: DropdownType) => {
  const SelectRef = useRef<HTMLDivElement>(null)
  const inputRef = useRef<HTMLInputElement>(null)
  const [showItems, setshowItems] = useState<boolean>(false)
  const [localOptions, setlocalOptions] = useState<OptionType[]>(options)
  const [localValue, setlocalValue] = useState<string>(value ? value.label : "Select")
  ...
  return (
    <div className='relative max-w-xs' {...rest} ref={SelectRef} id='select-container'>
      <div
